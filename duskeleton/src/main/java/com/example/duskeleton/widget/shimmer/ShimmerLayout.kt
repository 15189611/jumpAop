...
class ShimmerLayout @JvmOverloads constructor(
    ...
    fun startShimmerAnimation() {
        ...
        maskAnimator = getShimmerAnimation()
        maskAnimator?.start()
    ...
    private fun dispatchDrawShimmer(canvas: Canvas) {
        ...
        if (canvasForShimmerMask == null) {
            canvasForShimmerMask = Canvas(localMaskBitmap ?: return)
        }

        canvasForShimmerMask?.drawColor(
            Color.TRANSPARENT,
    ...
    private fun drawShimmer(destinationCanvas: Canvas) {
        ...
        destinationCanvas.drawRect(
            ...
            0f,
            maskRect?.width()?.toFloat() ?: 0F,
            maskRect?.height()?.toFloat() ?: 0F,
            gradientTexturePaint ?: return
        )
    ...
    private fun createShimmerPaint() {
        ...
        val maskBitmapShader = BitmapShader(
            localMaskBitmap ?: return,
            Shader.TileMode.CLAMP,
            Shader.TileMode.CLAMP
        )
    ...
    private fun getShimmerAnimation(): ValueAnimator? {
        ...
        animationFromX = if (width > maskRect?.width() ?: 0) {
            -animationToX
        } else {
            val valueWidth = maskRect?.width() ?: 0
            -valueWidth
        }
        ...
        maskAnimator?.addUpdateListener(ValueAnimator.AnimatorUpdateListener { animation ->
