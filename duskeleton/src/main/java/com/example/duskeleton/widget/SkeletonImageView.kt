...
class SkeletonImageView @JvmOverloads constructor(
    ...
    init {
        val typedArray =
            context.theme.obtainStyledAttributes(
                attrs,
                R.styleable.SkeletonImageView,
                defStyleAttr,
                0
            )
        val customScaleType =
            ScaleType.getType(typedArray.getString(R.styleable.SkeletonImageView_scaleType))
        ...
        scaleType = when (customScaleType) {
            ...
            ScaleType.FIT_TOP -> {
                ImageView.ScaleType.MATRIX.also {
                    matrixType = MatrixType.FIT_TOP
                }
            }
            ScaleType.FIT_BOTTOM -> {
                ImageView.ScaleType.MATRIX.also {
    ...
    private enum class ScaleType(val id: String) {
        ...
        companion object {
            fun getType(id: String?) = values().firstOrNull { it.id == id } ?: FIT_CENTER
        }
    }
